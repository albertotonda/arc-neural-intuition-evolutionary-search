...
def parse_expression(expr: str) -> Tuple[str, List[str]]:
    ...
    Handles terminal nodes that may be prefixed with underscore in the node tree.
    """
...
def get_normalized_name(name: str, check_tree_entry: bool = False) -> str:
    """Get the normalized version of a name, handling underscore prefixes."""
    clean_name = name.lstrip('_')
    if check_tree_entry:
        return f"_{clean_name}"
    return clean_name

def match_node_in_tree(node_name: str, possible_names: List[str]) -> bool:
    """Check if a node name matches any of the possible names, considering underscore prefixes."""
    if node_name in possible_names:
        return True
    if f"_{node_name}" in possible_names:
        return True
    return False
...
def calculate_expression_log_probability(expr: str, node_tree: Dict, forbidden: Set[str]) -> Tuple[float, ProbStep]:
    ...
    def filter_valid_lists(children_lists: List[List[str]], forbidden: Set[str]) -> List[List[str]]:
        ...
        return [children for children in children_lists 
                if not any(child.lstrip('_') in forbidden for child in children)]
    ...
    def calc_node_log_probability(expr: str, depth: int = 0) -> Tuple[float, ProbStep]:
        ...
        # If no parentheses, treat as terminal node
        if not args:
            normalized_name = get_normalized_name(fname, check_tree_entry=True)
            step = ProbStep(
                depth=depth,
                expr=expr,
                operation="Terminal node",
                log_prob=0.0,
                details=f"Terminal node '{fname}' (normalized as '{normalized_name}') - probability 1.0 (log10_prob = 0.0)",
                children=[]
            )
            return 0.0, step
            
        # Check if function exists in tree
        ...
        # Find exact matching children lists among valid ones
        matching_lists = []
        for child_list in valid_children_lists:
            if len(child_list) == len(args):
                ...
                for i, arg_value in enumerate(args):
                    ...
                    normalized_arg = get_normalized_name(arg_name, check_tree_entry=True)
                    if normalized_arg != child_list[i] and arg_name != child_list[i]:
                        is_valid = False
                        break
                if is_valid:
    ...
    for func_name, children_lists in node_tree.items():
        if func_name != 'root':
            ...
            for children in children_lists:
                all_keywords.update(child.lstrip('_') for child in children)
