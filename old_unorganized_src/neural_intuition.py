...
class GridDataset(Dataset):
    def __init__(self, data, max_size=None):
        self.data = data
        if max_size is None:
            self.max_size = max(max(G1.shape[0], G2.shape[0]) for G1, G2, _ in data)
        else:
...
class EnhancedGridClassifier(nn.Module):
    def __init__(self, num_transformations):
        ...
        # Second block: capture medium-scale features
        ...
        # Third block: capture global relationships
...
def generate_structured_grid(size):
    """Generate a grid with highly distinctive patterns."""
...
def generate_single_transform_data(num_samples=200, min_size=5, max_size=30):
    ...
    for _ in range(num_samples):
        ...
        transformations = np.zeros(5)
        transform_idx = np.random.randint(0, 5)
...
def generate_multiple_transform_data(num_samples=200, min_size=5, max_size=30):
    ...
    for _ in range(num_samples):
        ...
        transform_indices = np.random.choice(5, num_transforms, replace=False)
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            for loader_name, test_loader in test_loaders.items():
                test_loss, test_accuracy, _ = evaluate_model(model, test_loader, criterion, device, loader_name)
                test_losses.append(test_loss)
                if test_accuracy > best_test_accuracy:
                    best_test_accuracy = test_accuracy
                    torch.save(model.state_dict(), 'best_model.pth')
            
            # Update learning rate based on average test loss
...
def evaluate_model(model, dataloader, criterion, device, dataset_name=""):
    ...
    transformation_names = ['Rotation', 'Flip', 'Transpose', 'Add_1', 'Multiply_2']
...
if __name__ == "__main__":
    ...
    train_dataset = GridDataset(train_data, max_size=MAX_GRID_SIZE)
    single_test_dataset = GridDataset(single_transform_test, max_size=MAX_GRID_SIZE)
    multiple_test_dataset = GridDataset(multiple_transform_test, max_size=MAX_GRID_SIZE)
